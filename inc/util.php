...
changes: I am using util.php from canvas and changing one thing within pwdValidate.
Instead of password being longer than 12 characters, password has to be 10 or more characters.
Also added a function called isCodeInCorrectFormat() to see if the activation code is in the 
correct format.
...
function futureYearList($y){
      ...
      $list = "";
      for($i = $thisYear-1; $i < ($thisYear + $y); $i++){
             if ($i == $y)
                $list = $list."<option value=\"".$i."\" selected = \"selected\">".$i."</option>" ;
             else  $list = $list."<option value=\"".$i."\">".$i."</option>" ;
      }
...
// This function checks the format of the activation code
function isCodeInCorrectFormat($code)
{

	
	//code does not have 50 characters
	if(strlen($code) != 50)
	{
		return false;
	}
	...
	//go through each character of the code
